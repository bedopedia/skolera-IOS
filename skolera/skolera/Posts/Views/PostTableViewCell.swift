    ...
    var post: Post! {
        didSet {
            if post != nil {
                ...
                postImageView.childImageView(url: self.post.owner?.avatarUrl ?? "", placeholder: "\(String(post.owner?.firstname?.first ?? Character(" ")) )\(String(post.owner?.lastname?.first ?? Character(" ")))", textSize: 14)
